...
const tagPalette = [
  ["#e8eef2", "#aebfc9", "#344955"], ["#f1eae4", "#cdb9aa", "#5a4436"],
  ["#e8eee7", "#b3c2b0", "#3f533d"], ["#eee9f0", "#c3b3c8", "#514057"],
  ["#f1eee2", "#cbc39d", "#574f2f"], ["#e9eeee", "#adc0c0", "#395050"],
];
const tagStyle = (tag) => {
...
function renderTags() {
  ...
  tagsEl.innerHTML = ["", ...todas].map((tag) => `<button class="${tag === tagAtiva ? "active" : ""}${tag ? " colored" : ""}" ${tag ? `style="${tagStyle(tag)}"` : ""} data-tag="${escapeHtml(tag)}">${tag || "Todos"}</button>`).join("");
...
function renderLivros() {
  ...
  grid.innerHTML = filtrados.map((l, index) => {
    ...
    const inner = `<div class="cover cover-${index % 3}">${capa}</div><div class="card-info"><strong>${escapeHtml(l.titulo)}</strong><span>${escapeHtml(l.autores.join("; "))}${l.ano ? ` · ${l.ano}` : ""}</span><div>${l.tags.map((t) => `<small style="${tagStyle(t)}">${escapeHtml(t)}</small>`).join("")}</div></div>`;
